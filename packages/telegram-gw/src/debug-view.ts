...
export function renderDebugTail(info: DebugInfo): string {
  ...
  if (info.guardian) {
    lines.push(
      `🛡️ Защита: круг ${info.guardian.cycle} · правил ${info.guardian.rulesChecked} · нарушений ${info.guardian.violations}`,
    )
  }

  if (info.mcp) {
    const last = info.mcp.last ? ` (последний: ${escapeHtml(info.mcp.last)})` : ''
    lines.push(`🔌 MCP: ${info.mcp.servers.map(escapeHtml).join(' · ')}${last}`)
  }

  if (info.memory) {
    lines.push(`📊 Память: записей ${info.memory.episodic} · по смыслу ${info.memory.semanticK}`)
  }
