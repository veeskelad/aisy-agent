  ...
  switch (ev.kind) {
    ...
    case 'cost.summary': {
      ...
      const modelLine = isTiered
        ? `   Токены: ${ev.tokensIn} вход / ${ev.tokensOut} выход · $${ev.dollars.toFixed(3)} (по разной цене — разбивка по моделям в 📡 Монитор)`
        : `   Токены: ${ev.tokensIn} вход / ${ev.tokensOut} выход · ${escapeHtml(ev.model)}`
      return {
        html: [
    ...
    case 'spend.report': {
      ...
      if (ev.rows.length === 0) {
        ...
        for (const r of ev.rows) {
          lines.push(`• <b>${escapeHtml(r.model)}</b> — $${r.dollars.toFixed(3)} (${r.tokensIn} вход / ${r.tokensOut} выход)`)
        }
    ...
    case 'settings.panel': {
      // Значения живут на самих кнопках. Печатать их ещё и строками — это тот
      // же список дважды подряд, ровно то, на что жаловался оператор.
      const mark = (value: boolean): string => (value ? '✅' : '❌')
      return {
        html: '⚙️ <b>Настройки</b>',
        buttons: [
          [btn(`Стоимость за ход ${mark(ev.showCostPerTurn)}`, 'set:showCostPerTurn')],
          [btn(`Бюджет агентов ${mark(ev.budgetEnabled)}`, 'set:budgetEnabled')],
          [btn(`Отладка ${mark(ev.debug)}`, 'set:debug')],
        ],
      }
    }
  }
