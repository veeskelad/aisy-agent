...
describe('resolveTap', () => {
  it('reject is handled adapter-side without touching the gateway', async () => {
    ...
    if (out.kind === 'rejected') expect(out.footer).toContain('❌ Отменено')
  ...
  it('confirm echoes the button nonce and the displayed action-hash into the tap', async () => {
    ...
    const { deps, taps } = makeDeps(async () => ({
      decision: 'confirmed', actionId: 'act-001', proof,
    }))
    ...
    if (out.kind === 'confirmed') {
      ...
      expect(out.footer).toContain('✅ Разрешено · 14:32:01')
