...
describe('SteerQueue', () => {
  ...
  it('keeps transport metadata aligned while draining newest-first', () => {
    const q = new SteerQueue<{ updateId: number }>()
    q.enqueue(['first'], 100, { updateId: 10 })
    q.enqueue(['second'], 200, { updateId: 11 })

    expect(q.drain()).toEqual([
      { texts: ['second'], enqueuedAt: 200, metadata: { updateId: 11 } },
      { texts: ['first'], enqueuedAt: 100, metadata: { updateId: 10 } },
    ])
  })
