...
describe('renderDebugTail', () => {
  ...
  it('renders guardian, mcp, memory and journal lines', () => {
    ...
    expect(out).toContain('🛡️ Защита: круг 3 · правил 14 · нарушений 0')
    expect(out).toContain('🔌 MCP: filesystem · git (последний: read_file 47ms)')
    expect(out).toContain('📊 Память: записей 3 · по смыслу 5')
