...
describe('1. until + probe-pass → completed', () => {
  it('completes when claim is true and probe passes', async () => {
    ...
    expect(runGoalTurn).toHaveBeenCalledWith(expect.objectContaining({ binding: BINDING }))
  })
})
...
// ---------------------------------------------------------------------------
// 12. Live progress — the card the operator watches between the two terminal lines
// ---------------------------------------------------------------------------
describe('12. progress reporting', () => {
  it('publishes the goal before the first turn and after every iteration', async () => {
    const spec = makeSpec()
    let turns = 0
    const seen: Array<{ iterations: number; status: string }> = []
    const deps = makeDeps({
      runGoalTurn: async () => (++turns < 3 ? okContinue() : okDone()),
      reportProgress: (s) => { seen.push({ iterations: s.iterationsSpent, status: s.status }) },
    })

    await makeGoalOrchestrator(deps).start(spec, new AbortController().signal)

    // Before the first turn the goal has run nothing yet — that first report is
    // the whole point: a long first iteration is otherwise pure silence.
    expect(seen).toEqual([
      { iterations: 0, status: 'active' },
      { iterations: 1, status: 'active' },
      { iterations: 2, status: 'active' },
      { iterations: 3, status: 'completed' },
    ])
  })

  it('keeps working when the transport cannot show the card', async () => {
    const spec = makeSpec()
    const deps = makeDeps({
      runGoalTurn: async () => okDone(),
      reportProgress: () => { throw new Error('Telegram unreachable') },
    })
  ...
  it('reports the terminal state of an every-mode tick as well', async () => {
    const spec = makeSpec({ mode: { kind: 'every', intervalMs: 1000 } })
    const seen: string[] = []
    const deps = makeDeps({
      runGoalTurn: async () => okDone(),
      reportProgress: (s) => { seen.push(s.status) },
    })
    const orch = makeGoalOrchestrator(deps)

    await orch.start(spec, new AbortController().signal)
    await orch.tick(new AbortController().signal)
