...
import { ContextLeaseError } from '@aisy/core'
import {
  buildSpansWithRecall,
  detectLanguage,
  readTurnRecall,
...
describe('replyLanguageInstruction', () => {
  ...
  // A letterless message ("?", "👍", "42") names no language. Reading it as
  // non-Russian flipped the whole reply to the English system prompt's language.
  it('detects the language signal, or none for a letterless message', () => {
    expect(detectLanguage('ты где?')).toBe('ru')
    expect(detectLanguage('where are you')).toBe('other')
    expect(detectLanguage('?')).toBeNull()
    expect(detectLanguage('👍')).toBeNull()
    expect(detectLanguage('42')).toBeNull()
  })

  it('keeps the sticky language when the message carries no letters', () => {
    expect(replyLanguageInstruction('?', 'ru')).toBe('Отвечай на русском языке.')
    expect(replyLanguageInstruction('👍', 'ru')).toBe('Отвечай на русском языке.')
  })

  it('emits no instruction when there is no signal and no sticky language', () => {
    expect(replyLanguageInstruction('?')).toBe('')
  })

  // The message's own signal always beats the sticky one — a real switch still works.
  it('lets an explicit language in the message override the sticky one', () => {
    expect(replyLanguageInstruction('reply in english please', 'ru')).toBe(
      'Reply in the same language the operator used in their message.',
    )
  })
})
...
describe('readTurnRecall', () => {
  it('keeps ordinary recall failure best-effort', async () => {
    await expect(readTurnRecall(async () => { throw new Error('index cold') }, 'query'))
      .resolves.toBe('')
  })

  it('propagates stale context so the model cannot run under an old lease', async () => {
    await expect(readTurnRecall(
      async () => { throw new ContextLeaseError('STALE_CONTEXT') },
      'query',
