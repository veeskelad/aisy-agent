...
const TOOLS: AnthropicTool[] = [
  { name: 'read_file', description: 'Read a UTF-8 file from the workspace and get its contents. Use this to inspect a file before you answer about it or edit it. Always read before you describe what a file contains. Arg `path` is relative to the workspace root.', input_schema: { type: 'object', properties: { path: { type: 'string' } }, required: ['path'] } },
  { name: 'write_file', description: 'Create or overwrite a workspace file with the full new contents. Use after you have the complete text. Args: `path` (relative), `content` (the entire file). Overwriting an existing file is irreversible and may show the operator an approval card.', input_schema: { type: 'object', properties: { path: { type: 'string' }, content: { type: 'string' } }, required: ['path', 'content'] } },
