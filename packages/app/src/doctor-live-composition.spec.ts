...
describe('production doctor composition', () => {
  it('wires current memory, transcription, scheduler, MCP and optional Docker semantics', () => {
    ...
    expect(cli).toContain('compactMediaInboxWriterRecoveryArchives({')
    expect(cli).toContain('makeNodeMediaRecoveryRetentionPort({')
    expect(cli).toContain('retention: mediaRecoveryRetention')
expect(cli).toContain('recovery.compactArchives({')
expect(cli).toContain('const inbox = open()')
expect(cli).toContain('classifyMediaInboxStartupRefusal(error)')
expect(cli).toContain('renderMediaInboxStartupRefusal(refusal)')
    expect(cli).toContain("journal.append('media', 'media.writer_recovery_retained', retention)")
