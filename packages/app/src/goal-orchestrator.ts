...
export interface GoalOrchestratorDeps {
  ...
  recordGrant: (tool: string, binding: ResolvedWorkBinding) => void
  sendProgress: (text: string) => Promise<void>
  /**
   * The live state of the goal, published after every iteration and once before
   * the first one. A goal can run for many minutes between its two terminal
   * lines, and silence in a chat reads as a hung agent. Best-effort by
   * construction: a transport failure here must never stop the work.
   */
  reportProgress?: (spec: GoalSpec) => void | Promise<void>
  clock: { now: () => string }
...
export function makeGoalOrchestrator(deps: GoalOrchestratorDeps): GoalOrchestrator {
  const {
    store, runGoalTurn, probeRunner, recordGrant, sendProgress, reportProgress, clock, emit,
  } = deps
  ...
  // Same guard, plus the promise that reporting cannot break the loop.
  const report = async (s: GoalSpec): Promise<void> => {
    if (reportProgress === undefined || current?.id !== s.id) return
    try {
      await reportProgress(s)
    } catch { /* the work matters, the card does not */ }
  }
  ...
  async function iterate(
    ...
    if (r.state === 'awaiting-approval') {
      ...
      touch(spec)
      await persist(spec)
      await sendProgress('⏸ Цель остановлена: шаг опасный и требует твоего кода подтверждения. Сама себе я его не дам — сделай шаг сам или запусти цель заново.')
      return 'halted'
    }
  ...
  return {
    ...
    async start(spec: GoalSpec, signal: AbortSignal): Promise<void> {
      ...
      for (const tool of spec.grantedScope) {
        recordGrant(tool, spec.binding)
      }

      // Initial persist
      touch(spec)
      await persist(spec)
      // Before the first turn, so the operator sees the goal was taken rather
      // than nothing at all while an iteration runs.
      await report(spec)
