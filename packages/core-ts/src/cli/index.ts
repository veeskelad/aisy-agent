...
export interface CliDeps {
  ...
  color?: boolean
  /** Optional secure local ingress; absent until a production vault backend is configured. */
  credentialEntry?: {
    readSecret(prompt: string): Promise<Uint8Array>
    submit(entryCode: string, secret: Uint8Array): Promise<{
      ok: boolean
      errorCode?: string
    }>
  }
...
Usage:
  ...
  aisy doctor [--fix] [--json] [--post-upgrade]     Full-stack health check (read-only by default)
              [--only=a,b] [--skip=a,b]
  aisy update [--rollback | --cleanup | --allow-rewrite=<sha>]
                                                    Managed Git update (App distribution only)
  aisy diagnostics [--out=path]                     Write a redacted support bundle
  aisy brain credential set --code=<one-use-code>  Enter an API key through an interactive local TTY
  ...
  switch (command) {
    ...
    case 'update': {
      if (Object.keys(flags).length !== 0 || positional.length !== 0) {
        err('update: managed flags require the @aisy/app binary')
        return 2
      }
    ...
    case 'brain': {
      if (positional.length !== 2 || positional[0] !== 'credential' || positional[1] !== 'set' ||
        Object.keys(flags).some((key) => key !== 'code') || typeof flags['code'] !== 'string' ||
        !/^[A-Za-z0-9_-]{16,32}$/.test(flags['code'])) {
        err('brain credential: expected `aisy brain credential set --code=<one-use-code>`')
        return 2
      }
      if (deps.credentialEntry === undefined) {
        err('brain credential: secure credential backend is not configured')
        return 2
      }
      let secret: Uint8Array | null = null
      try {
        secret = await deps.credentialEntry.readSecret('API credential')
        const result = await deps.credentialEntry.submit(flags['code'], secret)
        if (!result.ok) {
          err(`brain credential: ${result.errorCode ?? 'API_CREDENTIAL_FAILED'}`)
          return 1
        }
        out('brain credential: validated and activated')
        return 0
      } catch {
        err('brain credential: secure entry failed')
        return 1
      } finally {
        secret?.fill(0)
      }
