...
class OpenAIError extends Error implements ProviderError {
  ...
  constructor(
    ...
    message: string,
    httpStatus?: number,
    brokerAttempt?: { attempted: boolean; attemptId?: string },
  ) {
    super(message)
    this.name = 'ProviderError'
    if (httpStatus !== undefined) this.httpStatus = httpStatus
    if (brokerAttempt !== undefined) {
      this.attempted = brokerAttempt.attempted
      if (brokerAttempt.attemptId !== undefined) this.attemptId = brokerAttempt.attemptId
    }
  }
}
  ...
  return {
    async complete(req: ModelRequest, signal?: AbortSignal): Promise<ModelResponse> {
      ...
      try {
        ...
        const brokerAttempt = err !== null && typeof err === 'object' &&
          typeof (err as { attempted?: unknown }).attempted === 'boolean'
          ? {
              attempted: (err as { attempted: boolean }).attempted,
              ...((err as { attemptId?: unknown }).attemptId === undefined
                ? {}
                : { attemptId: String((err as { attemptId: unknown }).attemptId) }),
            }
          : undefined
        throw new OpenAIError(
          kind,
          `OpenAI-compat request failed: ${(err as Error)?.message ?? 'unknown'}`,
          undefined,
          brokerAttempt,
        )
      }
