...
export const RUNTIME_TOOL_CATALOG = [
  ...
  { name: 'remember', description: 'Save one durable fact to long-term memory. Arg `fact` is the exact statement to store and show back; for a fact about the operator write it naturally in second person, e.g. `ты предпочитаешь краткие отчёты`. Legacy `text` is accepted temporarily, but never send both. On success preserve the returned `Запомнил, что <fact>` acknowledgement unchanged. Optional `topic`: `name`, `work`, `projects`, `style`, `autonomy` or `expectations`.', input_schema: objectSchema({ fact: { type: 'string' }, text: { type: 'string' }, topic: { type: 'string' } }), tier: 1, outboundSink: false, effect: 'write' },
  ...
  { name: 'configure_agent', description: 'Change or inspect one supported agent setting through a closed typed operation. Session operations: `session.rename`, `session.request-delete`, `session.propose-name`. Policy operations: `policy.resolve-path` (target `current`, value Project-relative directory or `.` to inspect policies), `policy.tighten-project` / `policy.tighten-path`, and `policy.relax-project` / `policy.relax-path`; path changes use the one-turn handle returned by `policy.resolve-path`. Policy mode is `ask-before-delete`, `confirm-writes`, `read-only` or `no-egress`. Folder scope supports `confirm-writes` and `read-only`; the other modes apply to the whole Project. Tightening is immediate; every relaxation gets an irreversible-action confirmation and never creates a grant. This tool cannot edit Aisy source code, prompts, Safety rules or the capability catalogue.', input_schema: objectSchema({ operation: { type: 'string' }, target: { type: 'string' }, value: { type: 'string' } }, ['operation', 'target']), tier: 1, outboundSink: false, effect: 'write' },
