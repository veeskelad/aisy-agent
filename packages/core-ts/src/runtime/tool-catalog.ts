...
export const RUNTIME_TOOL_CATALOG = [
  ...
  { name: 'spawn_subagent', description: 'Delegate a scoped task or goal-DAG plan to a sub-agent. Arg `plan` is JSON.', input_schema: objectSchema({ plan: { type: 'string' } }, ['plan']), tier: 2, outboundSink: false, effect: 'delegate' },
