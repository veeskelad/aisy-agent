...
export const RUNTIME_TOOL_CATALOG = [
  ...
  { name: 'configure_agent', description: 'Change one supported agent setting through a closed typed operation. Supported now: `session.rename` (target `current` or a handle from list_sessions; value is the new name), `session.request-delete` (target `current` or a handle; opens the real Telegram confirmation card but never deletes by model text), and `session.propose-name` (target `current`; value is a concise meaningful name for a newly temporary session after the first substantive operator request). A proposed name is applied only after the answer is delivered; do not mention this internal step in the reply. This tool cannot edit source code, prompts, safety policy or the capability catalogue.', input_schema: objectSchema({ operation: { type: 'string' }, target: { type: 'string' }, value: { type: 'string' } }, ['operation', 'target']), tier: 1, outboundSink: false, effect: 'write' },
