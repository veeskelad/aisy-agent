...
export interface Settings {
  ...
  debug: boolean
  /**
   * The operator's IANA timezone. Schedules are wall-clock promises, so
   * «каждое утро в 9» means 9 here. Empty ⇒ whatever the host runs in.
   */
  timeZone: string
}
...
/** Only the flags a menu tap can flip; `timeZone` is chosen, not toggled. */
