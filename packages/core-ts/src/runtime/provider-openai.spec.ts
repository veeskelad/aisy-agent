...
describe('makeOpenAICompatProvider.complete', () => {
  ...
  it('preserves broker attempt identity on ambiguous transport failure', async () => {
    const failure = Object.assign(new Error('PROVIDER_TRANSPORT_REFUSED'), {
      attempted: true,
      attemptId: 'request_' + 'a'.repeat(32),
    })
    const provider = makeOpenAICompatProvider({
      apiKey: '',
      model: 'gpt-test',
      baseUrl: 'https://api.openai.com/v1',
      fetchImpl: (async () => { throw failure }) as typeof fetch,
    })
    await expect(provider.complete(req([span('user', 'ping')])))
      .rejects.toMatchObject({ attempted: true, attemptId: failure.attemptId })
  })
