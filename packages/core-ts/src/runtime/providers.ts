...
const CLAUDE_CLI_MODELS = ['default', 'opus', 'sonnet', 'haiku']

export const PROVIDER_CATALOG: readonly ProviderEntry[] = [
  { id: 'anthropic', label: 'Anthropic (Claude API)', kind: 'anthropic', keyEnv: 'AISY_PROVIDER_ANTHROPIC_KEY', defaultModels: ['claude-opus-5', 'claude-sonnet-5', 'claude-haiku-4-5'] },
  ...
  // The CLI takes an alias rather than a full model id, and `default` means
  // "whatever the CLI picks" — the app drops the `--model` flag for that value.
  { id: 'claude-cli', label: 'Claude CLI (no API key)', kind: 'cli', cliCommand: ['claude', '-p'], defaultModels: CLAUDE_CLI_MODELS },
  // Subscription brain with real tools: the app composes it over the local MCP
  // bridge, so `buildProvider` never builds this entry (it would only produce a
  // toolless CLI provider). Listed as `cli` because it needs no API key.
  { id: 'claude-subscription', label: 'Claude Pro/Max (подписка, с инструментами)', kind: 'cli', cliCommand: ['claude', '-p'], defaultModels: CLAUDE_CLI_MODELS },
  // The app-server chooses the account's current default model unless the
  // operator explicitly pins one. Tools still execute only through Aisy MCP.
  { id: 'codex-subscription', label: 'ChatGPT/Codex (подписка, с инструментами)', kind: 'cli', cliCommand: ['codex', 'app-server'], defaultModels: ['default'] },
]
