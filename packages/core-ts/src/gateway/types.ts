...
export interface PendingAction {
  ...
  /** Code-owned similarity projection exists; transport may show remember buttons. */
  canRememberSimilar?: boolean
}

/**
 * Scope of an approval (ADR-0047). 'once' = this action only; 'session' /
 * 'always' ask the safety layer to remember a code-derived similar matcher.
 * Tier-3 (step-up) cards never carry a remembered scope — the confirmer drops it.
 */
...
/**
 * Code-minted evidence that the exact card/action passed nonce, hash and any
 * required step-up checks. Optional on generic approval adapters for backward
 * compatibility; permanence consumers must require and validate it.
 */
...
export interface GatewayDeps {
  ...
  cardTtlMs?: number
  /** Проверяющий второго фактора (код/пароль) для тира 3. Не задан — второго
   *  фактора нет, и тап оператора подтверждает карточку сам по себе. */
