...
import { GLOBAL_DNA_MEMORY_PATHS } from '../memory/global-dna.js'
...
/** Read-only media inbox writer-lock evidence supplied by the app runtime. */
export interface MediaInboxProbe {
  writerLock(): {
    /** `abandoned` — захват оборвался между `mkdir` и записью владельца. */
    state: 'absent' | 'held' | 'abandoned' | 'corrupt'
    archivedRecoveries: number
  }
}

/** Read-only evidence for the durable Telegram execution-card projection. */
export interface TelegramExecutionCheckpointProbe {
  inspect(): {
...
export interface MemoryPort {
  ...
  integrityCheck(): Promise<{ ok: boolean; detail?: string; repairable?: boolean }>
...
export interface RestrictedCloneSandboxReadiness {
  enablement: 'disabled' | 'enabled' | 'invalid'
  workerImageReferenceValid: boolean
  gatewayImageReferenceValid: boolean
  serverVersion: string | null
  versionCompatible: boolean
  workerImagePresent: boolean
  gatewayImagePresent: boolean
}
...
export interface SandboxProbe {
  daemonUp(): boolean
  /** Whether the legacy child-owned sandbox check is configured. Parent recovery is separate. */
  required?(): boolean
  /** Optional richer read-only diagnostic; callers must not expose raw stderr. */
  daemonStatus?(): DockerDaemonStatus
  imagePresent(): boolean
  runtime(): 'gvisor' | 'standard' | null
  capsDropped(): boolean
  /** Optional until restricted clone is configured; read-only and secret-free. */
  restrictedClone?(): RestrictedCloneSandboxReadiness
}
...
export interface MigrationReadinessProbe {
  workspaceV2(): {
    state:
      /** Реестра v1 нет вовсе — переносить нечего (установка родилась на v2). */
      | 'not-required'
      | 'not-prepared'
...
export interface NightlyPort {
  ...
  /** Runtime-owned scheduling authority; absent keeps the legacy external timer contract. */
  scheduleKind?(): 'in-process' | 'external'
...
/**
 * Legacy per-tier provider keys. Nothing writes them any more — the brain is
 * connected in Telegram and lands in the vault under its own provider name —
 * but an existing install may still hold them, so they stay readable.
 */
export const LEGACY_TIER_KEYS: readonly string[] = [
  'AISY_PROVIDER_REASONING_KEY',
  'AISY_PROVIDER_CRITIQUE_KEY',
  'AISY_PROVIDER_ROUTINE_KEY',
]
...
/**
 * What `aisy init` actually writes into `.env`. Only the two values a human
 * still types by hand plus the optional paths: offering empty provider-key
 * slots invites the operator to fill in a file the runtime no longer reads.
 */
export const ENV_TEMPLATE_KEYS: readonly string[] = REQUIRED_ENV_KEYS
  .filter((key) => !LEGACY_TIER_KEYS.includes(key))

/** The fixed scaffolding manifest (spec §4) — top-level files + memory tree.
 * All ADR-0063 global DNA files live exclusively under memory/ so they are
 * co-located with memoryRoot and readable by readFrozenSnapshot(). */
export const SCAFFOLD_FILES = [
  '.env',
  'AGENTS.md',
] as const

export const MEMORY_TREE_FILES = GLOBAL_DNA_MEMORY_PATHS

export const MEMORY_TREE_DIRS = ['memory/working', 'memory/daily', 'memory/archive'] as const

/** Read-only view of the session-journal writer lease (ADR-0068). */
export interface TranscriptWriterLeaseProbe {
  lease(): { state: 'absent' | 'held' | 'corrupt' }
}

/** Read-only readiness of the parent-owned Docker startup recovery barrier. */
export interface OwnedDockerRecoveryReadinessProbe {
  inspect(): Promise<{
    state:
      | 'disabled'
      | 'ready'
...
/** Read-only readiness of the root-owned native provider broker. */
export interface ProviderBrokerReadinessProbe {
  inspect(selectedProviders: readonly string[]): {
...
export interface OnboardingDeps {
  ...
  mcp: McpProbe
  /** Optional read-only v2 migration/rollback readiness probe. */
  migration?: MigrationReadinessProbe
  /** Optional read-only media inbox writer-lock probe. Never performs recovery. */
  mediaInbox?: MediaInboxProbe
  /** Optional read-only execution-card checkpoint probe. Never performs recovery. */
  telegramExecution?: TelegramExecutionCheckpointProbe
  /** Optional read-only session-journal writer-lease probe. Never performs recovery. */
  transcriptWriter?: TranscriptWriterLeaseProbe
  /** Optional read-only parent Docker recovery readiness. Never acquires writer authority. */
  ownedDockerRecovery?: OwnedDockerRecoveryReadinessProbe
  /** Optional read-only native provider broker readiness. Never performs vendor I/O. */
