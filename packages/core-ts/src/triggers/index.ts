...
import { ContextLeaseError } from '../runtime/context-lease.js'
import { wallClockIso } from '../runtime/wall-clock.js'
...
/** How many recent due-slots one trigger remembers for crash-safe idempotency. */
const MAX_FIRED_SLOTS = 16
/** Upper bound on live triggers: proactivity must not become a fork bomb. */
export const MAX_LIVE_TRIGGERS = 128
...
function isoWeekSlot(nowIso: string): string | null {
  const at = Date.parse(nowIso.slice(0, 10) + 'T00:00:00Z')
  if (Number.isNaN(at)) return null
  const day = new Date(at)
  // ISO week: Thursday of the same week decides which year the week belongs to.
  const weekday = (day.getUTCDay() + 6) % 7
  day.setUTCDate(day.getUTCDate() - weekday + 3)
  const firstThursday = Date.UTC(day.getUTCFullYear(), 0, 4)
  const firstWeekday = (new Date(firstThursday).getUTCDay() + 6) % 7
  const weekOne = firstThursday - firstWeekday * 86_400_000 + 3 * 86_400_000
  const week = Math.round((day.getTime() - weekOne) / (7 * 86_400_000)) + 1
...
function cronSlot(cron: string, nowIso: string): string | null {
  ...
  if (cron === '@weekly') return isoWeekSlot(nowIso)    // YYYY-Www
...
export function makeTriggerEngine(deps: TriggerEngineDeps): TriggerEngine {
  ...
  // Idempotency needs the recent past, not the whole history: a @minutely
  // trigger would otherwise grow one entry per minute forever.
  const recordSlot = (spec: TriggerSpec, slot: string): void => {
    spec.firedSlots = [...(spec.firedSlots ?? []), slot].slice(-MAX_FIRED_SLOTS)
  }
  ...
  const phase1 = async (
    ...
    if (spec.kind === 'schedule' && spec.cron) {
      // A schedule is a wall-clock promise: «в 9 утра» means the operator's 9,
      // not the server's. Only the slot is zoned — `now` stays UTC for
      // reminders, which are instants and must not move with a setting.
      const slot = cronSlot(spec.cron, wallClockIso(now, deps.timeZone?.()))
  ...
  const emitOnce = (spec: TriggerSpec, outcome: Phase1Outcome, payload: unknown): void => {
    ...
    if (outcome === 'context-paused') {
      if (pauseReported.has(spec.id)) return
      pauseReported.add(spec.id)
      deps.emitEvent('trigger.context_paused', payload)
      return
    }
  ...
  return {
    async register(input): Promise<TriggerSpec> {
      ...
      const live = [...triggers.values()].filter(s => s.enabled).length
      if (!triggers.has(input.id) && live >= MAX_LIVE_TRIGGERS) {
        throw new Error(`trigger rejected: at most ${MAX_LIVE_TRIGGERS} live triggers`)
      }
        ...
        if (p1.outcome === 'due' || p1.outcome === 'condition-met') {
          ...
          if (budgetExhausted(spec.budget) || budgetExhausted(deps.globalBackgroundBudget)) {
            ...
            try {
              ...
              if (!(error instanceof WorkBindingError) &&
                !(error instanceof ContextLeaseError)) throw error
              outcome = 'context-paused'
              spec.enabled = false
              await persist(spec)
            }
          }
