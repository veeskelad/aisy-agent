...
export interface TriggerEngineDeps {
  ...
  /**
   * The operator's IANA zone, read per tick so a change takes effect without a
   * restart. Absent ⇒ UTC, which is what every schedule did before.
   */
...
export interface TriggerEngine {
  ...
  register(spec: Omit<TriggerSpec, 'schemaVersion' | 'confirmed' | 'enabled'>): Promise<TriggerSpec>
