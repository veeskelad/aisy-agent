...
import { RUNTIME_TOOL_CATALOG } from '../runtime/tool-catalog.js'
import type {
  ToolCall,
  ContextState,
  ToolResult,
...
describe('AC-04-1: base tool count < 20', () => {
  ...
  it('AC-04-1b: compat registry cannot expand the runtime waist', () => {
    const registry = makeToolRegistry({ hooks: makeDenyingHooks() })
    expect('register' in registry).toBe(false)
    // @ts-expect-error — diagnostics are intentionally not public API
    expect(registry.count()).toBe(RUNTIME_TOOL_CATALOG.length)
  })

  it('compat names and policy metadata derive exactly from the runtime catalog', () => {
    const registry = makeToolRegistry({ hooks: makeDenyingHooks() })
    // @ts-expect-error — test-only compatibility diagnostic
    const definitions = registry.compatDefinitionsForTest()
    expect(definitions.map((tool: { name: string }) => tool.name))
      .toEqual(RUNTIME_TOOL_CATALOG.map(tool => tool.name))
    expect(definitions.map((tool: { tier: number; outboundSink: boolean }) =>
      [tool.tier, tool.outboundSink]))
      .toEqual(RUNTIME_TOOL_CATALOG.map(tool => [tool.tier, tool.outboundSink]))
  })
})
...
describe('AC-04-2: byte-stable tool-definition block hash', () => {
  ...
  it('AC-04-2b: mutating a tool field changes the hash', () => {
    ...
    // @ts-expect-error — test-only compat hash seam is not public API
    registry.mutateCompatToolForTest('bash', { description: 'changed' })
    // @ts-expect-error — serializeHash() does not exist yet (red test)
...
describe('AC-04-6: outbound lockout with untrusted span in context', () => {
  it('AC-04-6: fetch_url with untrusted span returns deny + outbound_locked event', async () => {
    ...
    const hooks: Hooks = {
      async preToolUse(call, ctx) {
        if (ctx.hasUntrustedSpan) {
          ...
          const outboundTools = new Set<string>(['fetch_url', 'web_search'])
  ...
  it('AC-04-6b: web_search with untrusted span returns deny', async () => {
    const effects = makeEffectVerifier()
    const hooks: Hooks = {
      async preToolUse(call, ctx) {
        if (ctx.hasUntrustedSpan) {
    ...
    const call: ToolCall = {
      tool: 'web_search',
      args: { query: 'current result' },
      argProvenance: { query: 'operator' },
    }
    const result = await registry.execute(call, makeUntrustedCtx())
    expect(result.verdict!.kind).toBe('deny')
    effects.expectEffect('tool-call', 'tool.outbound_locked')
...
describe('AC-04-7: lockout clears with clean operator turn (no untrusted span)', () => {
  it('AC-04-7: fetch_url returns allow when hasUntrustedSpan == false', async () => {
    const hooks: Hooks = {
      async preToolUse(call, ctx) {
        if (ctx.hasUntrustedSpan) {
          return { kind: 'deny', reason: 'outbound lockout', rule: 'OUTBOUND_LOCK' }
