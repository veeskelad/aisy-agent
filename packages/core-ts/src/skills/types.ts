...
export type ParseError =
  ...
  | { kind: 'prohibited_authority_field'; field: string }
  | { kind: 'empty_trigger' }
...
export interface StagedSkill {
  ...
  fullText: string
  /** Optional v2 identity; absent only for legacy callers. */
  revision?: SkillRevisionIdentity
  /** Legacy pre-preview hash; production uses approvalPreview().actionHash. */
  actionHash?: string
}
...
export interface SkillTraceEvidence {
  evidenceId: string
  skillName: string
  artifactHash: string
  revision: number
...
export interface SkillApprovalPreview extends ReviewCard {
  actionHash: string
...
export interface ApprovalVerdict {
  ...
  humanTapAuditId: string
  /** Optional for legacy cards; v2 cards must pin the canonical promotion action. */
  actionHash?: string
  /** Optional for legacy cards; v2 cards pin the exact real-trace evidence row. */
  traceEvidenceId?: string
  /** Mandatory in v2: exact timestamp of the pinned trace evidence row. */
  traceVerifiedAt?: string
  /** V2 audit timestamp; legacy cards may omit it. */
  approvedAt?: string
}

/**
 * promote() claims the staged record synchronously before its first mutating
 * await. A known external commit is finalized forward; an ambiguous commit
...
export interface Skills {
  ...
  /** Production approval preview. Fails closed without exact trace evidence. */
  approvalPreview(stageId: string): Promise<SkillApprovalPreview>
...
export interface ObservabilityPort {
  ...
  /** V2 exact-evidence lookup. When supplied, name-only fallback is not used. */
  passingTraceEvidence?(identity: SkillRevisionIdentity): Promise<SkillTraceEvidence | null>
...
export interface NonceStore {
  ...
  /** V2 exact action binding; legacy stores continue receiving stageId above. */
  consumeAction?(nonce: string, actionHash: string): boolean
}

export interface SkillPromotionAuditPort {
  record(binding: SkillPromotionAudit): Promise<void>
}
...
export interface SkillsDeps {
  ...
  /** Optional for backward compatibility; production composition must provide it. */
  promotionAudit?: SkillPromotionAuditPort
  /** Explicit compatibility escape hatch. Never enable for production approval previews. */
  unsafeLegacyPromotion?: boolean
}
