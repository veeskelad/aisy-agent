...
const ENTRY_FULL: McpServerEntry = {
  ...
  tools: [
    { tool: 'search', tier: 0, outboundSink: false, riskClass: 'readOnly', summary: 'Search issues' },
  ],
}
...
describe('MCP component (07)', () => {
  ...
  it('resolves immutable human policy without process or network I/O', () => {
    const deps = makeDeps()
    const manager = makeMCPManager(deps)
    const args = { q: 'test', nested: { limit: 3 } }

    const resolved = manager.resolve('tracker.search', args)

    expect(resolved).toMatchObject({
      server: 'tracker', tool: 'search', outboundSink: false, tier: 0, riskClass: 'readOnly',
    })
    expect(resolved.args).toEqual(args)
    expect(resolved.args).not.toBe(args)
    expect(Object.isFrozen(resolved.args)).toBe(true)
    expect(Object.isFrozen(resolved.args['nested'])).toBe(true)
    expect(deps.spawnProcess).not.toHaveBeenCalled()
    expect(deps.fetchDescriptors).not.toHaveBeenCalled()
  })

  it('rejects non-JSON or oversize args during policy resolution before I/O', () => {
    const deps = makeDeps()
    const manager = makeMCPManager(deps)

    expect(() => manager.resolve('tracker.search', { when: new Date() })).toThrow(/JSON values/i)
    expect(() => manager.resolve('tracker.search', { q: 'x'.repeat(70_000) })).toThrow(/64 KiB/i)
    expect(deps.spawnProcess).not.toHaveBeenCalled()
  })
  ...
  it('AC-07-10: summary==null triggers quarantined generation pass; mcp.summary_quarantined emitted; raw description absent from menu', async () => {
    const entryNullSummary: McpServerEntry = {
      ...ENTRY_FULL,
      tools: [{ tool: 'search', tier: 0, outboundSink: false, riskClass: 'readOnly', summary: null }],
    }
    const emit = vi.fn()
    const deps = makeDeps({
  ...
  it('AC-07-13: ResolvedMcpCall carries outboundSink and tier from McpToolPolicy, not from descriptor', async () => {
    ...
    expect(resolvedCalls[0]!.riskClass).toBe('readOnly')
    expect(resolvedCalls[0]!.server).toBe('tracker')
  })

  it('re-runs pin and descriptor gates on the invocation handle before tools/call', async () => {
    const handle = makeHandle('verified-call')
    const invokeTool = vi.fn(async (received) => {
      expect(received).toBe(handle)
      return 'ok'
    })
    const deps = makeDeps({
      spawnProcess: vi.fn(() => handle),
      resolvePin: vi.fn(async (received) => {
        expect(received).toBe(handle)
        return ENTRY_FULL.pin
      }),
      fetchDescriptors: vi.fn(async (received) => {
        expect(received).toBe(handle)
        return [TOOL_A]
      }),
      invokeTool,
    ...
    await expect(makeMCPManager(deps).call('tracker.search', { q: 'safe' }))
      .resolves.toMatchObject({ provenance: 'untrusted', text: 'ok' })
    expect(deps.spawnProcess).toHaveBeenCalledTimes(1)
    expect(invokeTool).toHaveBeenCalledTimes(1)
    expect(handle.terminate).toHaveBeenCalledTimes(1)
  })

  it('blocks invocation when the live pin changes after startup approval', async () => {
    const handle = makeHandle('pin-mismatch')
    const invokeTool = vi.fn(async () => 'must-not-run')
    const deps = makeDeps({
      spawnProcess: vi.fn(() => handle),
      resolvePin: vi.fn(async () => 'v9.9.9@sha256:changed'),
      invokeTool,
    })
  ...
  it.each([
    ['pin', { pin: '' }, /no exact pin/i],
    ['descriptor hash', { descriptorHash: '' }, /no descriptor hash/i],
  ])('refuses call with no %s before process creation', async (_label, patch, message) => {
    const invokeTool = vi.fn(async () => 'must-not-run')
    const deps = makeDeps({
      allowlist: { servers: [{ ...ENTRY_FULL, ...patch }] },
      invokeTool,
    })
    await expect(makeMCPManager(deps).call('tracker.search', {})).rejects.toThrow(message)
    expect(deps.spawnProcess).not.toHaveBeenCalled()
    expect(invokeTool).not.toHaveBeenCalled()
  })

  it('blocks invocation and emits a diff when live descriptors rug-pull', async () => {
    const handle = makeHandle('descriptor-mismatch')
    const invokeTool = vi.fn(async () => 'must-not-run')
    const emit = vi.fn()
    const deps = makeDeps({
      spawnProcess: vi.fn(() => handle),
      fetchDescriptors: vi.fn(async () => [{ ...TOOL_A, description: 'changed after approval' }]),
      invokeTool,
      emit,
    })

    await expect(makeMCPManager(deps).call('tracker.search', { q: 'safe' }))
      .rejects.toThrow(/descriptor hash mismatch/i)
    expect(invokeTool).not.toHaveBeenCalled()
    expect(handle.terminate).toHaveBeenCalledTimes(1)
    expect((emit.mock.calls as [McpEvent][]).map(call => call[0]))
      .toEqual(expect.arrayContaining(['mcp.disabled_hash_mismatch', 'mcp.diff_card_emitted']))
  })
  ...
  it('AC-07-14: untrusted span in context + outboundSink:true → deny; invokeApproved never called', async () => {
    const sinkEntry: McpServerEntry = {
      ...ENTRY_FULL,
      name: 'notifier',
      tools: [{ tool: 'notify', tier: 1, outboundSink: true, riskClass: 'idempotent', summary: 'Send a notification' }],
    }
  ...
  it('transport policy refuses before spawn for every invariant (ADR-0067)', async () => {
    const cases: Array<[string, McpServerEntry, string]> = [
      ['relative binary', { ...ENTRY_FULL, command: ['tracker-server'] }, 'command-not-absolute'],
      ['control chars in argv', { ...ENTRY_FULL, command: ['/usr/bin/tracker-server', 'a\nb'] }, 'command-unsafe'],
      ['plaintext endpoint', {
        ...omitKey(ENTRY_FULL, 'command'),
        transport: 'streamable-http',
        endpoint: 'http://mcp.example.com/v1',
      }, 'endpoint-not-https'],
      ['credentials in endpoint', {
        ...omitKey(ENTRY_FULL, 'command'),
        transport: 'streamable-http',
        endpoint: 'https://user:pass@mcp.example.com/v1',
      }, 'endpoint-has-credentials'],
      ['malformed legacy approval', {
        ...ENTRY_FULL,
        legacyProtocol: { approved: true, approvedBy: 'operator', approvedAt: 'yesterday' } as never,
      }, 'legacy-not-approved'],
    ]

    for (const [label, entry, reason] of cases) {
      const deps = makeDeps({ allowlist: { servers: [entry] } })
      const result = await makeMCPManager(deps).connect('tracker')

      expect(result, label).toEqual({ kind: 'refused', reason })
      expect(deps.spawnProcess, label).not.toHaveBeenCalled()
      expect(deps.resolvePin, label).not.toHaveBeenCalled()
      expect(deps.fetchDescriptors, label).not.toHaveBeenCalled()
    }
  })
  ...
  it('AC-07-23: a server whose name contains a dot is resolved against the allowlist, not mis-split on the first dot', async () => {
    ...
    const serverAB: McpServerEntry = {
      ...
      name: 'a.b',
      command: ['/usr/bin/ab'],
      tools: [{ tool: 'x', tier: 0, outboundSink: false, riskClass: 'readOnly', summary: 'AB tool' }],
    }
    const resolvedCalls: ResolvedMcpCall[] = []
    const deps = makeDeps({
