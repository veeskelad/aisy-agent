...
export interface McpServerEntry {
  ...
  legacyProtocol?: McpLegacyEraApproval // absent = modern-only era (ADR-0067)
}

/** Human-owned approval of the legacy MCP protocol era for one server (ADR-0067 §3).
 *  Absent means modern-only. It is never derived from a server response. */
export interface McpLegacyEraApproval {
  approved: true
  approvedBy: string        // operator identity that signed the manifest entry
  approvedAt: string        // ISO-8601 instant, e.g. 2026-07-29T10:00:00Z
}
...
export interface McpToolPolicy {
  ...
  riskClass: "readOnly" | "destructive" | "idempotent" // human-owned; never descriptor-derived
  summary: string | null    // human-authored menu line; null -> quarantined-generated pass (Eng-9)
}

// ── Connect result ─────────────────────────────────────────────────────────────

/** Transport-policy refusals (ADR-0067); every one happens before spawn or network contact. */
export type McpTransportRefusal =
  | "transport-mismatch"
...
export type ConnectResult =
  ...
  | { kind: "refused"; reason: "not-allowlisted" | "no-pin" | "no-hash" | "pin-mismatch" | "token-unresolved" | McpTransportRefusal }
...
export interface ResolvedMcpCall {
  ...
  tier: 0 | 1 | 2 | 3
  riskClass: "readOnly" | "destructive" | "idempotent"
}
...
export interface McpClient {
  ...
  /** Canonical sha256 over the complete accepted descriptor, sorted by name (CSO-M2).
...
export interface McpManager {
  ...
  /** Resolve a namespaced tool to immutable human-owned policy without I/O. */
  resolve(namespaced: string, args: unknown): ResolvedMcpCall
