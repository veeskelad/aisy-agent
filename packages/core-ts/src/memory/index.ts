...
import { GLOBAL_DNA_PREFIX_FILES } from './global-dna.js'
...
export {
  ...
  GLOBAL_DNA_MEMORY_PATHS,
  type GlobalDnaPrefixFile,
} from './global-dna.js'
export {
  deriveDeterministicMemoryFactKey,
  type DeterministicMemoryFactKey,
} from './fact-key.js'
...
/**
 * The byte-stable `MEMORY.md` body for a set of live facts (§5.7): stable sort,
 * fixed UTC timestamps, `\n` endings, single trailing newline, no volatile ids.
 *
 * Pure and exported because the projection has two producers: the SQLite store
 * regenerating from its own rows, and the runtime writing the file after a
 * commit. Two hand-written copies of this format would drift, and the nightly
 * regeneration compares bytes.
 */
export function serializeFactIndex(
  facts: ReadonlyArray<{ factKey: string; validAt: string; text: string }>,
): string {
  // Codepoint order, not locale order: SQLite sorts these rows with BINARY
  // collation, and the two producers must agree byte for byte.
  const byCodepoint = (a: string, b: string): number => (a < b ? -1 : a > b ? 1 : 0)
  const ordered = [...facts].sort((a, b) =>
    byCodepoint(a.factKey, b.factKey) || byCodepoint(a.validAt, b.validAt) ||
    byCodepoint(a.text, b.text))
  const lines = ['# Memory index', '']
  for (const fact of ordered) {
    lines.push(`- ${fact.factKey.slice(0, 12)} ${fact.validAt} ${fact.text.replace(/\s+/g, ' ').trimEnd()}`)
  }
  return lines.join('\n').replace(/\n+$/, '') + '\n'
}
...
export function makeMemoryStore(deps: MemoryStoreDeps): Memory {
  ...
  const guardCheck = (
    ...
    const exactQuery = d.prepare('SELECT rowid FROM do_not_remember WHERE fact_key = ?')
    if (exactQuery.get(factKey) || (legacyFactKey !== factKey && exactQuery.get(legacyFactKey))) {
      return 'BLOCK'
    }
    ...
    const tokenSet = new Set([...tokens, ...legacyTokens])
    for (const row of dnrRows(d)) {
  ...
  return {
    ...
    // SESSION SNAPSHOT SOURCE — read fresh for each new Agent Loop session.
    // Agent Loop freezes the returned bytes per session and the durable
    // transcript restores those exact bytes after restart (ADR-0007/0063).
    ...
    async commit(op: MemoryOp, _ctx: { withinSession: boolean }): Promise<CommitResult> {
      ...
      try {
        ...
        if (op.op === 'NOOP') {
          ...
          const normalized = deriveDeterministicMemoryFactKey(op.text)
          const tokens = normalized.keyTokens
          const factKey = normalized.factKey
          const legacyTokens = normalized.legacyKeyTokens
          const legacyFactKey = normalized.legacyFactKey
          const guard = guardCheck(d, factKey, tokens, legacyFactKey, legacyTokens)
          if (guard === 'BLOCK') {
            result = {
              status: 'BLOCKED',
    ...
    async serializeMemoryIndex(): Promise<{ content: string; sha256: string }> {
      ...
      const content = serializeFactIndex(
        rows.map((r) => ({ factKey: r.fact_key, validAt: r.valid_at, text: r.text })),
      )
