...
import { GLOBAL_DNA_PREFIX_FILES, makeMemoryStore } from './index.js'
...
describe('search query sanitization', () => {
  ...
  it('indexes and safely searches Cyrillic terms surrounded by punctuation', async () => {
    const { deps } = makeDeps()
    const store = makeMemoryStore(deps)
    const committed = await store.commit(
      { op: 'ADD', text: 'Мой любимый город — Москва' },
      { withinSession: false },
    )

    const hits = await store.search('«Москва»')

    expect(hits.map((hit) => hit.id)).toContain(committed.factId)
  })

  it('uses distinct non-empty fact keys for different Russian facts', async () => {
    const { deps } = makeDeps()
    const store = makeMemoryStore(deps)
    const city = await store.commit(
      { op: 'ADD', text: 'Мой любимый город — Москва' },
      { withinSession: false },
    )
    const project = await store.commit(
      { op: 'ADD', text: 'Я работаю над проектом Ася' },
      { withinSession: false },
    )

    const live = await store.listLive()
    expect(live.map((fact) => fact.id)).toEqual(expect.arrayContaining([city.factId, project.factId]))
    expect(new Set(live.map((fact) => fact.factKey)).size).toBe(2)
    expect(live.every((fact) => fact.factKey.length === 64)).toBe(true)
  })

  it('keeps legacy empty-key forget entries fail-closed after Unicode key rollout', async () => {
    const { deps } = makeDeps()
    const store = makeMemoryStore(deps)
    const legacyEquivalent = await store.commit(
      { op: 'ADD', text: '!!!' },
      { withinSession: false },
    )
    await store.forget(legacyEquivalent.factId!, 'legacy empty-key fixture', true)

    const result = await store.commit(
      { op: 'ADD', text: 'Секретный русский факт' },
      { withinSession: false },
    )

    expect(result.status).toBe('BLOCKED')
    expect(result.verdict).toMatchObject({ decision: 'BLOCK', matched: 'forget_list' })
  })
})
...
describe('AC-03-4: source snapshot and per-session freeze', () => {
  it('keeps source bytes stable when only a within-session database commit occurs', async () => {
    const { deps } = makeDeps()
    const store = makeMemoryStore(deps)
  ...
  it('assembles all global DNA in ADR-0063 order and refreshes the next session candidate', async () => {
    const { deps } = makeDeps()
    mkdirSync(deps.memoryRoot, { recursive: true })
    for (const name of GLOBAL_DNA_PREFIX_FILES) {
      writeFileSync(join(deps.memoryRoot, name), `[${name}:v1]`, 'utf8')
    }
    const store = makeMemoryStore(deps)

    const first = await store.readFrozenSnapshot()
    expect(first.bytes.toString('utf8')).toBe(
      GLOBAL_DNA_PREFIX_FILES.map(name => `[${name}:v1]`).join(''),
    )

    writeFileSync(join(deps.memoryRoot, 'MISSION.md'), '[MISSION.md:v2]', 'utf8')
    const nextSession = await store.readFrozenSnapshot()

    expect(first.bytes.toString('utf8')).toContain('[MISSION.md:v1]')
    expect(nextSession.bytes.toString('utf8')).toContain('[MISSION.md:v2]')
    expect(nextSession.sha256).not.toBe(first.sha256)
  })
})
