...
export function makeConsolidationRunner(deps: ConsolidationDeps): ConsolidationRunner {
  ...
  const normalizedLog = (snapshot?: NormalizedDayLog): NormalizedDayLog => {
    const raw = snapshot ?? deps.rawDayLog ?? { date: '2026-06-11', records: [] }
  ...
  const liveFacts = (facts: Fact[]): Fact[] => facts
    .filter((f) => f.invalidAt === null)
    .map((fact) => structuredClone(fact))

  const loadRunSnapshot = async (): Promise<ConsolidationRunSnapshot> => {
    if (deps.loadRunSnapshot === undefined) {
      return { facts: deps.facts ?? [], validators: deps.validators, ...(deps.rawDayLog === undefined ? {} : { rawDayLog: deps.rawDayLog }) }
    }
    try {
      const snapshot = await deps.loadRunSnapshot()
      if (!Array.isArray(snapshot.facts) || typeof snapshot.validators?.check !== 'function' ||
        (snapshot.rawDayLog !== undefined &&
          (typeof snapshot.rawDayLog.date !== 'string' || !Array.isArray(snapshot.rawDayLog.records)))) {
        throw new Error('invalid snapshot')
      }
      return {
        facts: snapshot.facts.map((fact) => structuredClone(fact)),
        validators: snapshot.validators,
        ...(snapshot.rawDayLog === undefined
          ? {}
          : { rawDayLog: structuredClone(snapshot.rawDayLog) }),
      }
    } catch {
      emit('night.snapshot.unavailable')
      throw new NightlyError('night.snapshot.unavailable: live state unavailable — fail-closed')
    }
  }
  ...
  const runConsolidation = async (
    ...
    for (const rawOp of proposal.ops) {
      ...
      const verdict = validators.check(op)
  ...
  const runLintPassInternal = async (log = normalizedLog()): Promise<LintPassResult> => {
    ...
    try {
      await deps.generator.draftSkills(log)
    } catch {
      return {
        orphans: [],
        staleAnnotations: [],
