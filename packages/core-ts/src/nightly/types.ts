...
export interface ConsolidationDeps {
  ...
  facts?: Fact[]
  /**
   * Optional per-run authority snapshot. Loaded once while the exclusive night
   * lock is held, then reused by generator and validators for the whole run.
   * This supersedes static rawDayLog/facts/validators for run(); the static
   * fields remain for backwards-compatible tests and runLintPass().
   */
  loadRunSnapshot?(): Promise<ConsolidationRunSnapshot>
...
export interface ConsolidationRunSnapshot {
  facts: Fact[]
  validators: Validators
  rawDayLog?: NormalizedDayLog
}
