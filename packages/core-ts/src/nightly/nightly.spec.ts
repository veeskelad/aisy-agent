...
describe('Nightly Consolidation', () => {
  ...
  it('loads one fresh fact/validator snapshot per run while holding the night lock', async () => {
    const calls: string[] = []
    const snapshots = [
      [makeLiveFact('fact-a', makeFactKey('fact-a'), 'old a')],
      [makeLiveFact('fact-b', makeFactKey('fact-b'), 'old b')],
    ]
    const generator: Generator = {
      proposeMemoryOps: async (_log, facts) => {
        calls.push(`generator:${facts.map((fact) => fact.id).join(',')}`)
        const target = facts[0]!
        return {
          ops: [{ kind: 'UPDATE', factId: target.id, factKey: target.factKey, text: `new ${target.id}` }],
    ...
    const runner = makeConsolidationRunner({
      ...
      loadRunSnapshot: async () => {
        ...
        const facts = snapshots.shift() ?? []
        const ids = new Set(facts.map((fact) => fact.id))
        return {
          facts,
          validators: {
            check(candidate) {
              if (!('kind' in candidate) || candidate.kind === 'ADD' || candidate.kind === 'NOOP') {
                return { ok: true }
              }
              return ids.has(candidate.factId) ? { ok: true } : { ok: false, failed: ['refs_exist'] }
            },
          },
        }
      },
    ...
    const first = await runner.run(defaultConfig)
    const second = await runner.run(defaultConfig)

    expect(first.card.memoryEdits[0]?.summary).toContain('fact-a')
    expect(second.card.memoryEdits[0]?.summary).toContain('fact-b')
    expect(calls).toEqual([
      'lock:acquire', 'snapshot:load', 'generator:fact-a', 'lock:release',
      'lock:acquire', 'snapshot:load', 'generator:fact-b', 'lock:release',
    ])
  })

  it('fails snapshot loading closed, releases the lock and redacts the source error', async () => {
    const release = vi.fn()
    const generator = { ...deps.generator, proposeMemoryOps: vi.fn(deps.generator.proposeMemoryOps) }
    const runner = makeConsolidationRunner({
      ...deps,
      generator,
      lock: {
        acquire: () => ({
          ok: true,
          token: { pid: 1, bootId: 'boot', startTime: 0, nonce: 'n', acquiredAt: 0 },
        }),
        release,
      },
      loadRunSnapshot: async () => { throw new Error('private memory path and bytes') },
    })
    ...
    expect(message).toBe('night.snapshot.unavailable: live state unavailable — fail-closed')
    expect(message).not.toContain('private memory path and bytes')
    expect(generator.proposeMemoryOps).not.toHaveBeenCalled()
