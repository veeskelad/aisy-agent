...
export class DelegationResumeError extends Error {
  constructor(
    public readonly code: DelegationQuarantineReason | 'NOT_RESUMABLE',
    message: string,
  ) {
    super(message)
    this.name = 'DelegationResumeError'
  }
}
...
export interface AgentCard {
  name: string
  description?: string
  /** Markdown body: role, constraints and agent-specific DNA. */
  instructions: string
...
export type DelegationStatus = 'active' | 'completed' | 'failed' | 'resumed'

/**
 * Versioned durable snapshot of one delegation. The persistence adapter may
 * split this logical record into manifest/checkpoint/shard files, but Core
 * validates the complete record before exposing a resumable handle.
 */
export interface PersistedDelegationV1 {
  schemaVersion: 1
  runId: RunId
  binding: ResolvedWorkBinding
  task: DelegationTask
  ...
  /** Required for terminal state so parent delivery survives process restart. */
  terminalObservation?: TaskObservation
}

/** Shared run ledger; never reconstructed from a possibly stale child record. */
export interface PersistedDelegationRunV1 {
  schemaVersion: 1
  runId: RunId
  binding: ResolvedWorkBinding
  runBudget: IterationCost
...
export type DelegationQuarantineReason =
  | 'legacy-or-invalid-state'
  | 'binding-mismatch'
  | 'binding-inactive'
  | 'task-or-card-mismatch'
  | 'shard-integrity-failed'
  | 'checkpoint-mismatch'

/** Raw persistence boundary: Core owns validation and fail-closed semantics. */
export interface DelegationPersistencePort {
  loadRun(): unknown
  load(delegationId: DelegationId): unknown
  saveRun(state: PersistedDelegationRunV1): void
  save(state: PersistedDelegationV1): void
  quarantine(
    delegationId: DelegationId,
...
export interface DelegationHandle {
  delegationId: DelegationId
  taskId: string
  /** Immutable context inherited from the parent lease. */
  binding: ResolvedWorkBinding
...
export interface DelegationDeps {
  /** Exact durable context inherited from the parent turn. */
  binding: ResolvedWorkBinding
  ...
  /** Emit an orchestration event to Observability (12). */
  emit(event: OrchestrationEvent): void
  /** Optional durable store. Omitted keeps the existing in-memory behaviour. */
  persistence?: DelegationPersistencePort
  /** Lifecycle authority. Resume fails closed when the bound context is archived. */
...
export type DelegationRecoveryResult =
  | { status: 'terminal'; observation: TaskObservation }
  | { status: 'resumable'; handle: DelegationHandle }

export type DelegationRecoveryPreflight =
  | Readonly<{ status: 'none' }>
  | Readonly<{
      status: 'terminal' | 'continuation'
      activeDelegationIds: readonly DelegationId[]
      terminalObservations: readonly TaskObservation[]
      runBudget: Readonly<IterationCost>
    }>
...
export interface DelegationManager {
  ...
  /** Startup recovery: replay terminal state or resume an interrupted active shard. */
  recover(delegationId: DelegationId): DelegationRecoveryResult
  /**
   * Cold, read-only full-inventory validation. It installs no state, emits no
   * event, quarantines nothing and never issues a handle.
   */
  preflightRecovery(): DelegationRecoveryPreflight
  /**
   * Atomically recover the exact persisted active set. Every durable child is
   * validated before any handle is exposed; partial/replayed sets are refused.
   */
  ...
  /** Replay-safe compact terminal result; never exposes the child transcript. */
