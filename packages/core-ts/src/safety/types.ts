...
export interface ToolCall {
  ...
  argsTainted?: boolean
  /** Code-owned dynamic tier resolved before HookGate (for example MCP policy). */
  policyTier?: Tier
  /** Code-owned outbound classification; server annotations are never authority. */
...
// ---------------------------------------------------------------------------
// Scoped approval grants — legacy v2 tool-wide + v3 similar (ADR-0047/0093)
// A grant may only suppress a tier-based `ask`; it can NEVER override a `deny`.
// Tier-3 is never grantable (step-up every time).
// ---------------------------------------------------------------------------

export type GrantScope = 'session' | 'always'

/** Durable context identity carried by every v2 approval grant. */
...
export interface PersistedApprovalGrantV2 {
  schemaVersion: 2
  tool: string
  scope: 'always'
...
/** Durable code-derived matcher. Resource identity is hashed so paths, ids and
 * command operands do not become plaintext runtime state. */
export interface PersistedSimilarApprovalGrantV3 {
  schemaVersion: 3
  kind: 'similar'
  tool: string
  operation: string
...
export interface GrantPersistenceStateV3 {
  schemaVersion: 3
  similarGrants: PersistedSimilarApprovalGrantV3[]
  /** Existing v2 tool-wide grants remain usable outside Plan Mode only. */
  legacyToolGrants?: PersistedApprovalGrantV2[]
  quarantinedLegacyTools?: string[]
  invalidRecordCount?: number
}

export interface GrantPersistenceStateV2 {
  schemaVersion: 2
  grants: PersistedApprovalGrantV2[]
  /** Legacy per-tool grants are retained for review but never authorize use. */
  quarantinedLegacyTools?: string[]
  invalidRecordCount?: number
}

/** Raw/versioned persistence. Parsing and fail-closed quarantine live in GrantStore. */
export interface GrantPersistencePort {
  load(): unknown
  save(state: GrantPersistenceStateV2 | GrantPersistenceStateV3): void
}

export interface GrantListEntry {
  tool: string
  scope: GrantScope
  kind?: 'legacy-tool' | 'similar'
  operation?: string
  resourceHashPrefix?: string
  binding?: GrantBinding
  status: 'active' | 'disabled'
  disabledReason?: 'context-unavailable' | 'legacy-unscoped' | 'policy-revision-mismatch'
}

export interface GrantStore {
  /** True only when a live grant covers this tool in this exact context. */
  has(tool: string, binding?: GrantBinding): boolean
  /** Record a grant. Missing/invalid binding fails closed. */
  record(tool: string, scope: GrantScope, binding?: GrantBinding): void
  /** True only for a code-derived v3 matcher covering this exact call. */
  hasSimilar(call: ToolCall, risk: Tier, binding?: GrantBinding): boolean
  /** Whether this call has a safe, bounded similarity projection. */
  canRememberSimilar(call: ToolCall, risk: Tier): boolean
  /** Record a session/durable v3 matcher. Missing/invalid binding fails closed. */
  recordSimilar(call: ToolCall, risk: Tier, scope: GrantScope, binding?: GrantBinding): void
  /** Revoke the matching context grant, or every grant for the tool when omitted. */
  revoke(tool: string, binding?: GrantBinding): void
  /** Revoke grants in one context, or every grant when omitted. */
  revokeAll(binding?: GrantBinding): void
  list(binding?: GrantBinding): GrantListEntry[]
}
...
export interface SandboxConfig {
  ...
  egressBridge?: string
  /** @deprecated Runner-owned probe is authoritative; caller value is ignored. */
