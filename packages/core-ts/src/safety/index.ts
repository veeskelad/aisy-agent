...
export { autonomyWorkflowStep, GrantStoreError, makeGrantStore } from './grants.js'
export type { GrantStoreDeps } from './grants.js'

import { httpsHost } from './grants.js'
...
function isOutboundOrSideEffecting(call: ToolCall): boolean {
  if (call.outboundSink === true) return true
...
function tierOf(call: ToolCall): Tier {
  if (call.policyTier !== undefined) return call.policyTier
...
/** Card text is read by a human: one line, no control characters, bounded. */
const CARD_ARG_CHARS = 120

function cardArgument(value: unknown): string | null {
 ...
 * authorize something they cannot see:
 *
 *  - a page fetch is approved per domain, so the domain goes on the card;
 *  - a research run is approved as a whole (ADR-0097), so the question goes on
...
function actionSummary(call: ToolCall): string {
  ...
  if (call.tool === 'deep_research') {
    const question = cardArgument(call.args['question'])
    if (question !== null) return `deep_research(${question})`
  }
  return `${call.tool}(${Object.keys(call.args).join(', ')})`
}

function makeCard(call: ToolCall, tier: Tier): ConfirmationCard {
  return {
    tier,
    actionSummary: actionSummary(call),
...
export interface SafetyPolicyDeps {
  ...
  /**
   * Approval grants (ADR-0047/0093). A v3 similar matcher, or compatibility v2
   * rule, may suppress Tier-2 `ask` only after every deny check; never Tier-3.
   */
  grants?: GrantStore
  /** Current immutable Workspace/Project/Session context for grant lookup. */
...
export function makeSafetyPolicy(deps: SafetyPolicyDeps = {}): SafetyPolicy {
  ...
  return {
    ...
    evaluate(call: ToolCall, ctx: ContextSpan[]): Verdict {
      ...
      const runtimePolicy = call as ToolCall & { policyTier?: unknown; outboundSink?: unknown }
      if ((runtimePolicy.policyTier !== undefined &&
        ![0, 1, 2, 3].includes(runtimePolicy.policyTier as number)) ||
        (runtimePolicy.outboundSink !== undefined &&
          typeof runtimePolicy.outboundSink !== 'boolean')) {
        return { decision: 'deny', rule: 'INVALID_DYNAMIC_POLICY', reason: 'invalid code-owned tool policy' }
      }
      ...
      // Tier-2 asks, unless a v3 similar or compatibility v2 grant applies —
      ...
      if (tier === 2) {
        if (deps.grants?.hasSimilar(call, tier, deps.grantBinding) === true ||
          deps.grants?.has(call.tool, deps.grantBinding) === true) {
...
export interface SandboxRunnerDeps {
  ...
  /** Probe for the gVisor (runsc) runtime; absence is fail-degraded. */
  gVisorProbe?: () => boolean
  ...
  /** Proves daemon-wide userns-remap or rootless mode before container creation. */
  usernsRemapProbe?: () => boolean
}
...
export function makeSandboxRunner(deps: SandboxRunnerDeps = {}): SandboxRunner {
  ...
  const gVisorAvailable = deps.gVisorProbe?.() ?? false
  const seccompProfile = deps.seccompProfile ?? 'builtin'
  const usernsRemapEnabled = deps.usernsRemapProbe?.() ?? false
  ...
  return {
    ...
    async start(config: SandboxConfig): Promise<string> {
      ...
      // Docker userns-remap is a daemon setting, not a `docker run` flag.
      // Absence of a positive daemon/rootless probe is fail-closed.
      if (!usernsRemapEnabled) throw new Error('sandbox: user namespace remap unavailable')
