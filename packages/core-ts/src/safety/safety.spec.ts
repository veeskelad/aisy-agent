...
describe('AC-05-3: Tier-2 ask below Delegation; Tier-3 always ask', () => {
  ...
  it('names the domain on a page-fetch card and keeps the path off it', () => {
    const policy = makeSafetyPolicy()
    const fetch: ToolCall = {
      ...call('fetch_url', { url: 'https://Example.com/deep/page?token=abc' }),
      policyTier: 2,
    }
    const verdict = policy.evaluate(fetch, [operatorSpan()])

    expect(verdict.decision).toBe('ask')
    if (verdict.decision === 'ask') {
      expect(verdict.card.actionSummary).toBe('fetch_url(example.com)')
      expect(verdict.card.actionSummary).not.toContain('token')
    }
  })

  it('falls back to argument names when a fetch url is unusable', () => {
    const policy = makeSafetyPolicy()
    const fetch: ToolCall = { ...call('fetch_url', { url: 'not a url' }), policyTier: 2 }
    const verdict = policy.evaluate(fetch, [operatorSpan()])

    if (verdict.decision === 'ask') expect(verdict.card.actionSummary).toBe('fetch_url(url)')
  })

  it('puts the research question on the card, in one bounded line', () => {
    // One tap authorizes the whole search (ADR-0097), so the operator has to
    // read what is being searched — not the word "question".
    const policy = makeSafetyPolicy()
    const research: ToolCall = {
      ...call('deep_research', { question: 'сколько стоит\n\n  перевозка кота  ' }),
      policyTier: 2,
    }
    const verdict = policy.evaluate(research, [operatorSpan()])

    expect(verdict.decision).toBe('ask')
    if (verdict.decision === 'ask') {
      expect(verdict.card.actionSummary).toBe('deep_research(сколько стоит перевозка кота)')
    }
  })

  it('bounds a long research question instead of flooding the card', () => {
    const policy = makeSafetyPolicy()
    const research: ToolCall = {
      ...call('deep_research', { question: 'я'.repeat(400) }),
      policyTier: 2,
    }
    const verdict = policy.evaluate(research, [operatorSpan()])

    if (verdict.decision === 'ask') {
      expect(verdict.card.actionSummary.length).toBeLessThan(160)
      expect(verdict.card.actionSummary).toContain('…')
    }
  })

  it('falls back to argument names when the research question is not text', () => {
    const policy = makeSafetyPolicy()
    const research: ToolCall = { ...call('deep_research', { question: '   ' }), policyTier: 2 }
    const verdict = policy.evaluate(research, [operatorSpan()])

    if (verdict.decision === 'ask') {
      expect(verdict.card.actionSummary).toBe('deep_research(question)')
    }
  })
...
describe('ADR-0047: scoped approval grants', () => {
  ...
  it('a session grant turns a Tier-2 ask into allow', () => {
    ...
    grants.record('bash', 'session', binding)
    const policy = makeSafetyPolicy({ grants, grantBinding: binding })
  ...
  it('without a grant the same Tier-2 call still asks', () => {
    const policy = makeSafetyPolicy({ grants: makeGrantStore(), grantBinding: binding })
    const verdict = policy.evaluate(call('bash', { cmd: 'npm test' }), [operatorSpan()])
    expect(verdict.decision).toBe('ask')
  })
  ...
  it('a grant NEVER overrides HARD_DENY (rm -rf stays denied)', () => {
    const grants = makeGrantStore()
    grants.record('bash', 'always', binding)
    const policy = makeSafetyPolicy({ grants, grantBinding: binding })
  ...
  it('a grant NEVER overrides the narrowed-outbound deny', () => {
    const grants = makeGrantStore()
    grants.record('telegram.send', 'always', binding)
    const policy = makeSafetyPolicy({ grants, grantBinding: binding })
  ...
  it('Tier-3 is never suppressed by a grant (step-up every time)', () => {
    const grants = makeGrantStore()
    grants.record('db.drop-database', 'always', binding)
    grants.recordSimilar(call('db.drop-database', { name: 'prod' }), 2, 'always', binding)
    const policy = makeSafetyPolicy({ grants, grantBinding: binding })
    const verdict = policy.evaluate(call('db.drop-database', { name: 'prod' }), [operatorSpan()])
    expect(verdict.decision).toBe('ask')
...
describe('AC-05-20: Sandbox security constraints applied', () => {
  it('AC-05-20: started container reports cap-drop ALL, no-new-privileges, seccomp profile, and user-namespace remap', async () => {
    ...
    // CSO-M1 / ADR-0012: seccomp is explicit; userns is daemon-probed, not a run flag.
    expect(flags.some(f => f.startsWith('--security-opt=seccomp='))).toBe(true)
    expect(flags.some(f => f.startsWith('--userns-remap='))).toBe(false)
    expect(flags).not.toContain('--userns=host')
  })
  ...
  it('AC-05-20: container creation fails closed without daemon userns evidence', async () => {
    const launches: string[][] = []
    const runner = makeSandboxRunner({
      usernsRemapProbe: () => false,
      launch: (_config, flags) => {
        launches.push(flags)
    ...
    await expect(runner.start({
      image: 'node:20', mounts: [], gVisorAvailable: true, taskId: 'task-no-userns',
    })).rejects.toThrow(/user namespace remap/i)
...
describe('AC-05-21: Degraded mode when gVisor absent', () => {
  ...
  it('AC-05-21: runner never trusts caller gVisor state over its probe', async () => {
    const fullLaunches: string[][] = []
    const full = makeSandboxRunner({
      gVisorProbe: () => true,
      usernsRemapProbe: () => true,
      launch: (_config, flags) => { fullLaunches.push(flags); return 'full' },
    })
    await full.start({
      image: 'node:20', mounts: [], gVisorAvailable: false, taskId: 'caller-false',
    })
    expect(full.securityLevel).toBe('full')
    expect(fullLaunches[0]).toContain('--runtime=runsc')

    const degradedLaunches: string[][] = []
    const degraded = makeSandboxRunner({
      gVisorProbe: () => false,
      usernsRemapProbe: () => true,
      launch: (_config, flags) => { degradedLaunches.push(flags); return 'degraded' },
    })
    await degraded.start({
      image: 'node:20', mounts: [], gVisorAvailable: true, taskId: 'caller-true',
    })
    expect(degraded.securityLevel).toBe('degraded-no-gvisor')
    expect(degradedLaunches[0]).not.toContain('--runtime=runsc')
  })
