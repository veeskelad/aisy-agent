...
> **Поправка от 2026-08-09.** Новые remembered approvals больше не создают
> per-tool grants: [ADR-0093](./2026-08-09-code-derived-similar-approval-grants.md)
> заменяет гранулярность на code-derived `tool + operation + resource + risk +
> policy revision`. Schema-v2 записи остаются legacy-совместимостью только для
> `auto` и не наследуются Plan Mode.

> **Поправка от 2026-07-27.** Пункты о гранулярности и формате хранения
> уточнены принятым ADR-0060 и русской спецификацией Workspace/Project context
> (§11). Durable grant больше не может быть записью только с именем инструмента:
> он обязан иметь `WorkBinding`. `session` ограничен точной сессией, `always` —
> исходным Workspace/Project/Session scope; global grant требует отдельного
> действия оператора. Legacy `{ "always": string[] }` сохраняется в выключенном
> карантине и не мигрирует в global capability. Остальные инварианты ADR-0047 —
> grants подавляют только Tier-2 prompt, не обходят deny и не применяются к
> Tier-3 — остаются без изменений.
