...
### Кто ставит цель (дополнение 2026-08-11)

Изначально цель существовала только как команда `/goal` с позиционным режимом —
на телефоне её никто не набирал, то есть слоя фактически не было. Дополнение
меняет две вещи и ни одной границы:

- **`set_goal`** — tier-1 инструмент узкой талии, которым модель *предлагает*
  цель: `objective` и необязательный `mode`. Он ничего не запускает. Живой порт
  проверяет режим до показа карточки, чтобы оператор не мог тапнуть в ошибку, и
  возвращает модели формулировку «показал карточку», а не «запланировал».
- **Карточка запуска** с двумя ответами. Цель начинает работать в момент
  создания, поэтому — в отличие от триггера (ADR-0029), который регистрируется
  спящим, — здесь до тапа не создаётся ничего. Карточка привязана к своему
  `message_id`: тап по устаревшему предложению не запускает работу.
- **Экран 🎯 Цели** показывает то, что до сих пор было видно только в ответе
  `/goal status`: цель, режим, израсходованные итерации и деньги против потолка,
  последнюю проверку. Кнопка остановки есть только у идущей цели.
- **Живая карточка** той же формы: оркестратор публикует состояние до первой
  итерации и после каждой, карточка редактируется на месте. Между двумя
  терминальными строками цель могла молчать минутами, и молчание в чате читается
  как зависший агент. Публикация best-effort по построению: сбой доставки не
  останавливает работу, а завершённая цель отпускает карточку — она остаётся
  отчётом, следующая цель заводит свою.

Backstop, read-only-scope по умолчанию, негрантируемость tier-3 и `/stop` не
меняются: предложение проходит ровно тот же путь, что и операторская команда.
