...
- The transcript of record is the private full-fidelity journal defined by
  ADR-0064 (durable session sequence and per-session hash chain). Observability
  receives a content-redacted projection. The Context Engine never UPDATEs or
...
- **Neutral:** a fifteenth component; `assemble` adds a summarizer model call on
  compaction (billed/capped via Provider 09); the ADR-0064 journal becomes the
  private transcript of record and Observability remains the redacted audit
  projection.
