...
- Live day-log normalization remains a follow-up. Live fact freshness закрыта
  2026-07-28: каждый `run()` под `night.lock` один раз получает свежие facts и
  согласованный с ними validator snapshot; long-running process больше не ждёт
  restart, чтобы увидеть новые факты.
