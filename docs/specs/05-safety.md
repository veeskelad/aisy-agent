...
Human permanence confirmation (ADR-0029). The model proposes a fact or deletion; Safety strips any
...
**Нормативное уточнение Docker user namespaces и gVisor.** `userns-remap`
настраивается и проверяется на уровне daemon/rootless Docker; это не параметр
`docker container run`. Runtime никогда не передаёт несуществующий
`--userns-remap` и запрещает `--userns=host`. До создания контейнера требуется
положительное code-owned свидетельство `name=userns` либо `name=rootless` из
bounded `docker info`. Результат code-owned probe gVisor является единственным
источником `securityLevel` и флага `--runtime=runsc`; caller не может объявить
...
19. **AC-05-19** — Sandbox start aborts (container not created) if the mount spec requests
    ...
    ADR-0012)
20. **AC-05-20** — До `container create` runtime получает положительное daemon
    evidence `userns`/`rootless`; запущенный sandbox по inspect подтверждает
    `cap-drop ALL`, `no-new-privileges` и применённый seccomp profile. В argv нет
    `--userns-remap` и `--userns=host`. (CSO-M1, ADR-0012)
...
28. **AC-05-28** — When the egress proxy is unavailable, no outbound request succeeds, and
    ...
    (ADR-0010, ADR-0012, §7)
29. **AC-05-29** — Узкий read-only `web_search` допускает только exact
    `html.duckduckgo.com:443`: каждый DNS-ответ публичный, один IP закреплён на
    единственный TLS-запрос и совпадает с remote peer, redirect/retry отсутствуют,
    secret-like query отклоняется до DNS, а DNS вместе с ответом укладываются в
    общий deadline и byte limits. Этот критерий не заменяет внешний общий egress
    proxy. `fetch_url` ходит через тот же гантлет без allowlist хостов и просит
    подтверждение на каждый новый домен (ADR-0094, AC-04-33, AC-04-34).
30. **AC-05-30** — Принятое оператором исключение ADR-0091 ограничено точным
    tool name `bash` и живым durable-режимом `bypass`: Safety возвращает allow
    без HARD_DENY/narrowing/approval только для него. В `auto/confirm/plan`, при
    ошибке чтения/записи режима и для любого другого tool прежние проверки
    остаются в силе. Состояние `bypass` восстанавливается только из exact schema
    v2 в приватном обычном файле; future/corrupt/symlink/broad-mode state и
    незавершённый переход всегда сужаются до non-bypass. UI показывает blast
    radius и способ выхода `/mode auto`.
31. **AC-05-31** — Durable «разрешить похожие» grant принимает только
    code-normalized selector `tool + operation + WorkBinding + resource + risk
    ceiling + policy revision`; model-authored matcher, selector mismatch,
    archived binding, higher risk и extreme-destructive action дают ask/deny,
    а legacy tool-only grant не мигрирует в similar capability автоматически.
32. **AC-05-32** — Для `write_file`/`write_knowledge` одинаковый tool и exact
    нормализованный path совпадают при другом content; другой path/project,
    risk выше ceiling или другая policy revision не совпадают. Durable state
    переживает restart, но не содержит raw path/content/operands. Простой Bash
    совпадает только по executable/subcommand и exact hashed operands; shell
    composition и `spawn_subagent` не получают remembered matcher.
...
34. **AC-05-34** — В режиме «сначала план» показанный план не даёт права ни на
    ...
    фазы `approved`, куда переводит решение оператора по **точному** `planHash`;
...
35. **AC-05-35** — `submit_plan` блокируется до ответа оператора. Отказ
    ...
    — план отзывается, а не остаётся ждать. Уже начатое действие нельзя ни
    переодобрить, ни отменить (`PLAN_EXECUTION_ACTION_AMBIGUOUS`). Отказы
    приходят модели фразами о том, что происходит и что делать, а не кодами
состояния (ADR-0100).
36. **AC-05-36** — Вне режима «сначала план» `submit_plan` показывает карточку и
...
- ADRs:
  ...
  - [ADR-0092 — Plan Mode как исследование перед автоматическим выполнением](../decisions/2026-08-09-plan-mode-research-then-execute.md)
  - [ADR-0093 — Code-derived similar grants](../decisions/2026-08-09-code-derived-similar-approval-grants.md)
- Concept docs:
