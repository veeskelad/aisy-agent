...
**Status:** Draft
**Component:** 01 / 17
**Related ADRs:** ADR-0005, ADR-0007, ADR-0008, ADR-0019, ADR-0026, ADR-0027, ADR-0059
...
- **Action Contracts** classify operator turns as answer-only, inspect-required,
  mutate-required, or delegate-required and prevent completion without code-observed
  evidence ([ADR-0059](../decisions/2026-07-26-action-contracts-verified-completion.md)).
...
export interface TurnResult {
  ...
  actionContractKind?: "inspect-required" | "mutate-required" | "delegate-required"
...
`action.contract`, `action.recovery`, `action.unverified`, `turn.end`. **Events consumed:** the HookGate verdict, the Provider adapter result, the
Loop Guardian verdict.

Для transport progress agent loop отдельно публикует code-owned события
`outbound-lockout`, `turn-started`, `tool-pending`, `tool-started`,
`tool-completed`, `tool-denied` и `tool-failed`. Provider callback имеет более
узкий тип и не может сформировать эти события. Tool lifecycle содержит только
монотонный sequence, имя capability и категорию `tool|subagent`; аргументы,
результаты и exception text в transport progress не входят. `tool-pending`
публикуется перед HookGate, `tool-started` — только после allow и непосредственно
перед executor, terminal lifecycle соответствует фактическому результату code
dispatch.
...
### 4.7 Action Contract and evidence

The classifier reads only operator-provenance user spans. Informational requests remain
`answer-only`; explicit inspect, mutate, and delegation verbs select their required
contract. Untrusted spans cannot manufacture an action requirement.

Evidence comes only from successful dispatch results or passing Plan Mode probes:

- inspect requires a successful observation-family tool;
- mutate requires a successful mutation followed by a later independent observation,
  unless the tool returns a typed `verified:true` or non-empty `receiptId`;
- delegate requires a successful `spawn_subagent` result;
- a fully passing externally probed plan satisfies the contract.

Model prose is never evidence. If evidence is missing after the normal tool/synthesis
loop, code issues exactly one constrained recovery model call. Hooks, approvals, budgets,
...
turn code-owned runtime ведёт exact фазы:

```text
research → submitted → attempted → completed
                         └ crash/unknown result → ambiguous
drift/failure → research (новая revision и новое research evidence)
```

- `research` допускает только catalog calls с read/search effect и durable
  считает только успешные наблюдения той же session/turn identity;
- internal `submit_plan` принимает не prose, а bounded exact call list;
- на диске остаются tool names и SHA-256 нормализованных args, но не raw args;
- до dispatch `attempted` публикуется атомарно, а genuine one-shot permit
  связывает terminal result с exact plan/step/revision;
- рестарт с `attempted` даёт `ambiguous` и zero automatic retry;
- повтор того же submission идемпотентен, completed step не открывается снова;
- другой tool/args/binding/policy revision возвращает задачу в `research` до
  любой мутации.

Durable backend активируется только через общий LIVE protocol §4.9; прямого
пути от provider call к state controller или executor нет.

### 4.9 Provider-neutral `submit_plan` protocol

Один и тот же LIVE protocol обслуживает обычный AgentRunner и внутренние
tool loops Claude/Codex subscription. Provider видит bounded internal
`submit_plan(plan: string)`, где строка содержит exact JSON из §4.8. Успешный
ответ `PLAN_ACCEPTED` означает «продолжай следующий шаг сейчас», а не «попроси
оператора переключить режим».

Preflight вызывается до HookGate/approval: в research он пропускает только
read/search calls, перехватывает `submit_plan` и отклоняет unplanned acting call
без executor I/O и без approval card. После обычного Safety/approval admission
повторно сверяет caller-detached exact call, durable публикует `attempted` и
только затем передаёт ту же копию executor. API и subscription provider
возвращают исходные transport-owned `sessionId/turnId`; provider-local identity
не является заменой. Успешный research read фиксируется только после
HookGate.post/output filtering; withheld result не создаёт research evidence.
...
33. **AC-01-33** — An informational operator request such as “Explain how to create a
    file” completes in one model call with no Action Contract system span and no
    `actionStatus`.
34. **AC-01-34** — “Check this file” with two prose-only model responses executes exactly
    one recovery model call, returns `actionContractKind:"inspect-required"` and
    `actionStatus:"unverified"`, and never returns the model's success claim.
35. **AC-01-35** — An inspect-required turn becomes `verified` only after a successful
    observation-family tool result.
36. **AC-01-36** — A mutate-required turn with a successful mutation but no typed receipt
    remains `unverified` until a later independent observation succeeds.
37. **AC-01-37** — A hook-denied action remains denied during recovery; no denied tool is
    executed and recovery performs at most one additional model call.
38. **AC-01-38** — A successful `spawn_subagent` result satisfies a delegate-required
    contract; absent execution does not.
39. **AC-01-39** — A lint-passing plan whose every external probe passes returns
    `actionStatus:"verified"` for an action-required operator turn.
40. **AC-01-40** — Разрешённый tool dispatch публикует exact последовательность
    `tool-pending → tool-started → tool-completed`; denied call заканчивается
    `tool-denied`, а exception или typed `{ok:false}` executor result —
    `tool-failed`; exception сохраняется для исходного control flow.
41. **AC-01-41** — `spawn_subagent` получает code-owned категорию `subagent`,
    остальные calls — `tool`; progress payload не содержит args, result или
    exception detail.
42. **AC-01-42** — Provider progress callback не может публиковать code-owned
    lifecycle/lockout типы, а отказ transport sink не меняет Safety, dispatch или
    итог turn.
43. **AC-01-43** — После каждого принятого `ModelResponse.usage` loop публикует
    code-owned `turn-usage` с накопительными token/dollar значениями до проверки
    mid-turn budget; исходный provider `usage` не является terminal authority.
44. **AC-01-44** — Action-required turn публикует code-owned `action-contract`,
    а перед единственной corrective model попыткой — `action-recovery` с
    перечислимой причиной недостающего доказательства; model callback не может
    выпускать эти события.
45. **AC-01-45** — `submit_plan` для exact session/turn без хотя бы одного
    успешного catalog read/search observation отклоняется, сохраняет phase
    `research` и даёт zero acting dispatch.
46. **AC-01-46** — После принятого bounded submission runtime допускает только
    следующий exact tool+normalized-args hash; mismatch возвращает новую
    research revision до HookGate/executor I/O.
47. **AC-01-47** — До acting dispatch durable state уже имеет `attempted`; crash
    до terminal receipt восстанавливается как `ambiguous` и не выполняет call
    повторно.
48. **AC-01-48** — Повтор byte-identical submission той же session/turn/plan
    identity после restart идемпотентен, а completed step остаётся completed.
49. **AC-01-49** — Plan Mode backend доступен production только через общий
    protocol; прямой вызов acting executor без preflight/admission отсутствует.
50. **AC-01-50** — При restart persisted plan принимается только если его
    research evidence, phase/step invariants и пересчитанный exact call-list
    hash согласованы; future, corrupt, accessor/Proxy и подменённое состояние
    дают code-only fail-closed до acting dispatch.
51. **AC-01-51** — Provider-neutral protocol до submission выполняет только
    catalog read/search calls; acting call и malformed `submit_plan` дают zero
    executor I/O, а successful read увеличивает research evidence.
52. **AC-01-52** — Accepted `submit_plan` не ждёт отдельного user start: следующий
    exact planned call проходит к executor, mismatch возвращает research до I/O.
53. **AC-01-53** — Между exact snapshot/admission и executor caller mutation,
    accessor, Proxy или custom iterator не меняют tool/args и не исполняют
    caller code.
54. **AC-01-54** — Thrown/malformed executor terminal после `attempted` не
    закрывает шаг; restart восстанавливает `ambiguous` и zero automatic retry.
55. **AC-01-55** — AgentRunner, Claude subscription и Codex subscription
    передают одному capability path исходные transport-owned `sessionId/turnId`;
    отсутствие `turnId` даёт zero Plan Mode tool I/O.
56. **AC-01-56** — LIVE composition устанавливает `submit_plan`, preflight и
    post-approval admission и на AgentRunner, и на Claude/Codex subscription;
    compaction/nightly/sub-agent providers внутренний tool не получают.
57. **AC-01-57** — Unplanned acting call в `plan` возвращает code-only отказ до
    approval card; exact planned опасный call проходит обычный approval и лишь
    после решения публикует `attempted` перед executor.
...
- ADRs: [ADR-0005](../decisions/2026-06-11-own-agent-loop.md),
  ...
  [ADR-0027](../decisions/2026-06-11-capability-narrowing-untrusted-context.md),
  [ADR-0059](../decisions/2026-07-26-action-contracts-verified-completion.md).
