...
- The **append-only session transcript** — every turn (spans, tool calls,
  ...
  the ADR-0064 private transcript journal; the Context Engine holds the working
  index and writes through to it. Observability receives a content-redacted
  event projection.
...
- It does **not** own persistence or audit projection — the full private trace
  lives in ADR-0064's transcript journal and Observability owns the
  content-redacted event stream. The Context Engine reads/writes through the
  transcript port and relies on it for the "view, not a write" guarantee.
...
interface TranscriptEntry {
  ...
  operatorId: string
  profileId: string
  projectId: string           // Workspace id or Project id
  sessionId: string
  sessionSeq: number          // durable ordering from ADR-0064
  ...
  rowHash: string
  loadBearing: boolean        // code-owned, hashed ADR-0064 field; unresolved bug / open decision / pending TODO — recall-first
}
...
interface TranscriptPort {
  /** Generates/verifies the complete ADR-0064 envelope and per-session chain; derives hashed loadBearing metadata after caller input validation. */
  append(
    lease: TurnContextLease,
    entry: Pick<TranscriptEntry, 'role' | 'provenance' | 'text'>,
  ): TranscriptEntry
  /** Rejects any envelope whose owner/profile/project/session differs from lease. */
  read(lease: TurnContextLease): TranscriptEntry[]
}

interface ContextEngineDeps {
  transcript: TranscriptPort
  /** Frozen prefix for this exact leased session — pinned, never compacted. */
  pinnedPrefix(lease: TurnContextLease): { text: string; breakpoints: number[] }
...
interface ContextEngine {
  ...
  assemble(lease: TurnContextLease): Promise<CompactionView>
  currentTier(lease: TurnContextLease): CompactionTier
}
```
...
Core (01), the transcript port from ADR-0064, and redacted events to Observability (12).

## 4. Data structures

**Append-only transcript** (§3) — entries are ADR-0064 full-fidelity envelopes
(bound to operator/profile/project/session, durable `sessionSeq`, per-session hash chain). The transcript port verifies every envelope against the immutable `TurnContextLease`; owner/scope/session/generation mismatch, sequence gap or hash failure is rejected before projection. The Context Engine never issues an
...
Every tier is a **view** — `transcript.read(lease)` always returns the full trace for that exact lease.
...
```
record(lease, turn) -> transcript.append(lease, envelope) -- append-only

assemble(lease)
  entries = transcript.read(lease)               -- verifies ADR-0064 envelope
  est = estimateTokens(pinnedPrefix(lease) + entries)
...
- **No destructive write.** `assemble` and every tier only read the transcript
  and emit segments; a post-compaction `transcript.read(lease)` returns the identical full
  trace (AC-15-2).
...
fact); ADR-0064 Transcript — the append-only private record; Observability
(12) — its content-redacted audit projection.
...
| Transcript unavailable | ADR-0064 store down | write/read error | **Fail-closed**: cannot guarantee the trace is preserved → do not compact (run verbatim if it fits, else refuse the turn) — never compact without a durable trace | journal recovers |
...
| **Audit erasure via compaction** | "compact away" an incriminating action | compaction is a VIEW; the full trace stays in the hash-chained journal — `transcript.read(lease)` is unchanged | ADR-0040, spec 12 |
...
1. **AC-15-1** — Under budget, `assemble(lease)` returns tier `none` and every transcript entry verbatim in order.
2. **AC-15-2** — After any compaction (`snip`…`auto`), `transcript.read(lease)` returns the byte-identical full transcript — compaction never mutated the trace.
...
10. **AC-15-10** — When the ADR-0064 transcript store is unavailable, `assemble(lease)` does **not** compact (fail-closed: no compaction without a durable trace).
...
12. **AC-15-12** — `record`, `assemble`, `currentTier`, transcript read and
    pinned-prefix access reject missing/stale/wrong-owner/wrong-profile/
    wrong-project/wrong-session leases; an ADR-0064 envelope mismatch,
    sequence gap or hash-chain failure is never projected.
