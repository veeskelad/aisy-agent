...
15. **AC-14-15** — `set_trigger` — the narrow-waist tool through which the model schedules its own work — carries only `kind`/`prompt` plus the one field that kind needs (`when`, `cron` or `probe`); an empty string is not an answer and never reaches the engine, an empty `prompt` is refused, and a runtime without the port answers `set_trigger: unavailable` rather than reporting a schedule it did not make.
16. **AC-14-16** — A trigger created through `set_trigger` is registered with `createdBy: 'agent'` and reaches the operator as a card with exactly two answers: confirming calls `confirm()` (AC-14-9) and refusing calls `cancel()` — a refused trigger is removed, not parked. Until one of the two is tapped the trigger is dormant, and the tool's return text says so instead of claiming the work is scheduled.
