...
Текущий production-preview runtime дополнительно закрывает входные и recovery
границы до запуска child. Payload shard проходит итеративный preflight глубины
не более 128 и размера не более 1 MiB до рекурсивной canonical serialization;
cycle, non-finite/non-JSON value и превышение лимита не мутируют shard. И Core,
и app runtime принимают DAG не более 256 задач, 2048 exact edges и 64
dependencies на задачу; все зависимости должны ссылаться на известные узлы,
edges обязаны точно совпадать с `dependsOn`, а граф — быть ацикличным.

При recovery terminal snapshot принимается только при exact parity с последним
runtime-owned terminal draft по status/summary/result/cost. Если durable run
ссылается на задачу, но её child snapshot отсутствует, задача не respawn-ится:
весь recovery закрывается. Code-owned meter является cumulative; verifier и
terminal cost не могут быть меньше уже измеренного child cost ни в одной
координате, включая путь подтверждённой cancellation во время verifier.
Live binary/run-id scan, реальные child/provider adapters и Telegram status
cards этим preview runtime не подключены.
...
21. **AC-11-21** — До mutation итеративный shard preflight отклоняет payload
    глубже 128 или больше 1 MiB без `RangeError`; DAG отклоняется при более чем
    256 tasks, 2048 edges, 64 dependencies, unknown/self/duplicate dependency,
    cycle или несовпадении exact `edges` и `dependsOn`.
22. **AC-11-22** — Recovered terminal observation обязана строго совпадать с
    runtime-owned terminal draft по status/summary/result/cost;
    referenced task без child snapshot даёт fail-closed recovery и zero respawn.
23. **AC-11-23** — Code-owned cumulative cost verifier/terminal не может быть
    меньше child cost по iterations/spend/wall; правило действует и после
    подтверждённой cancellation, а decreasing meter не создаёт terminal state.

Трассировка AC-11-21…23:
`packages/core-ts/src/orchestration/orchestration.spec.ts` и
`packages/app/src/durable-delegation-runtime.spec.ts`.
