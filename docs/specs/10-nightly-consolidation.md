...
- **Normalized day log** — the de-duplicated, timestamp-ordered stream of
  ...
  Реализованный exact-Session projector уже безопасно переносит проверенные
  `user`/`assistant` как `utterance` и `tool` как `tool-result`, исключая
  system/DNA/Skills и не угадывая структурированные события из текста. Его
  Node source требует exact Project/Session maintenance lease, ограничивает
  records/bytes и переживает restart. Неактивированный app-coordinator принимает
  только заранее сохранённый список точных Project/system-session bindings,
  по одному получает исключительный maintenance lease, дожидается drain
  операций этого Project и применяет обязательный forget-filter до выдачи
  снимка. Фильтр имеет право только удалить записи: изменение, перестановка,
  дублирование или фабрикация блокируются кодом. В live nightly coordinator пока
  не подключён. Concrete protected-memory adapter уже проверяет global и exact
  Project `do_not_remember` через общий deterministic fact-key normalizer,
  recovery gate и scope barriers; residual token overlap также удаляется
  fail-safe, а наружу выходят только счётчики. До versioned structured source
  `tool-call`/`decision-journal` такие record kinds блокируются, а не проходят
  без фильтра.
...
PROMOTION (on human Approve tap, via Safety 05 approval handler)
  for each approved staged patch:
    ...
    COMMIT ORDER (Eng-7):
      (1) Memory.commit(lease, ...) runs the scope-exclusive WAL:
          protected ledger -> canonical file -> FTS -> audit
      (2) async vector derivation may lag/fail without blocking keyword
      (3) git commit + push canonical files after WAL is AUDITED
...
**Crash-recovery ordering (Eng-7).** The protected ledger plus canonical file
published through Memory's operation-id WAL is authoritative and reaches
`AUDITED` before git commit/push. Vector derivation is disposable and not a
promotion prerequisite. The commit-journal entry is written
...
15. **AC-10-15** — Promotion calls `Memory.commit(lease, ...)` and proves every
    protected-ledger/file/FTS/audit WAL boundary before git commit/push. Vector
    derivation may be unavailable without blocking keyword publication. (Eng-7)
16. **AC-10-16** — Crash after memory WAL `AUDITED` but before git push resumes
    by operation id at git and produces exactly one commit; the protected row,
    canonical file, FTS identity and audit remain mutually consistent. (Eng-7)
17. **AC-10-17** — Crash at every pre-`AUDITED` WAL boundary is recovered or
    rolled back idempotently before scope reads resume, reapplying the
    resurrection guard; no ledger/file/FTS/audit resource is independently
    visible. (Eng-7)
...
36. **AC-10-36** — каждый `run()` после захвата `night.lock` ровно один раз
    получает свежий snapshot live facts и validator authority для тех же fact
    ids. Второй run того же процесса видит факт, добавленный после первого run;
    ошибка snapshot source освобождает lock, не вызывает generator/judge и
    возвращает только стабильный code без source detail.
37. **AC-10-37** — transcript day-log projection читает ровно один exact
    Project/Session hash chain, не смешивает Projects после restart и не
    превращает system/DNA/Skill rows в память.
38. **AC-10-38** — tool result с текстом, похожим на `tool-call` или
    `decision-journal`, остаётся только `tool-result`; отсутствующее
    структурированное событие не фабрикуется.
39. **AC-10-39** — stale/mismatched maintenance lease блокирует day-log source
    до filesystem I/O; corruption и overflow дают стабильный fail-closed отказ
    без private detail и без truncation.
40. **AC-10-40** — nightly читает Projects только из durable exact-binding
    snapshot; текущая interactive selection, registry enumeration и Workspace
    binding не расширяют список Projects, а Project A/B никогда не смешиваются.
41. **AC-10-41** — per-Project maintenance lease блокирует новые операции только
    этого Project, дожидается уже начатых операций и освобождает barrier после
    успеха или отказа; Workspace и другие Projects продолжают работать.
42. **AC-10-42** — обязательный ingestion forget-filter выполняется внутри того
    же lease и может вернуть только упорядоченное подмножество исходного day log;
    rewrite, reorder, duplicate или fabricated record дают fail-closed отказ до
    generator/judge и без смешивания Project snapshots.
43. **AC-10-43** — concrete protected-memory filter проверяет каждый text record
    одновременно против verified global и exact-Project forget verdict,
    удаляет exact/residual matches и не раскрывает keys/tokens/reasons/content в
