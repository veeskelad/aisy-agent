...
## Build progress (snapshot 2026-08-23)

The original 15-component engine has grown into 28 component specifications and
107 ADRs. The gap is no longer generic runtime wiring:

- ✅ **Production composition is connected** — Telegram, Projects/Sessions,
  scoped memory, tools, active Skills, stdio MCP, monitoring, provider and voice
  brokers, AgentCards and supervised durable sub-agents are imported by the
  production binary.
- ✅ **Deterministic safety remains code-owned** — approvals, grants, budgets,
  untrusted-input narrowing, exact-domain HTTPS policy, secret brokers and
  restart/rollback boundaries have adversarial and fault/restart coverage.
- ✅ **Managed delivery without APT is implemented** — managed Git provides
  atomic current/previous generations; privileged provider/voice releases use
  manifest-bound SSH bundles (ADR-0106, component 28).
- 🚧 **Remaining v0.1 work is release acceptance** — publish the clean public
