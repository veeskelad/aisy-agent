...
- Managed Git install/update/offline rollback with immutable generations and
  manifest-bound SSH delivery for privileged provider and voice sidecars.
- Clean public-snapshot publication boundary: historical private refs remain in
  a private archive while the canonical public repository starts from an
  audited root snapshot.

### Changed

- Production status documentation now distinguishes LIVE composition,
  dormant code, absent seams, ADR-deferred scope and target acceptance.
